...
// ------------
const createSemanticComponent = (tag: string) => styled(tag)`
	${props => semantics(props as SemanticProps)}
	${props => gridSemantics(props as SemanticProps)}
`;
...
export const getFeedback = (color: 'positive' | 'negative' | 'warning', opacity?: number) => (props: { theme: Theme }) => {
